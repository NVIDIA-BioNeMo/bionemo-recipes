...
Determine termini class, not merely linear versus circular. GenBank `LOCUS` is submitter-set and biologically unvalidated: OY979411.1 declares `circular`; it is a linear Drulisvirus deposited as a DTR-collapsed, circularly permuted repeat unit whose 127-nt overlap is an assembler k-mer artifact, not its roughly 250-nt DTR.
...
1. Test direct `seq[:k] == seq[-k:]` and inverted `seq[:k] == reverse_complement(seq[-k:])` matches. Reject common assembler k-mers (21, 31, 33, 55, 77, 99, 111, 127) and constant overlaps across unrelated sizes or families. With a few percent mismatch, real repeats extend with degradation; artifacts stop exactly at k. Validate the detector on a published DTR.
2. With no credible repeat, compare relatives: different rotations indicate headful permutation; consistent endpoints indicate fixed cos/cohesive ends (lambda).
...
5. No repeat in ssDNA Microviridae indicates a genuine circle; end CDS markers `<1..N` and `M..>L` with no origin-spanning `join()` indicate linear annotation.
