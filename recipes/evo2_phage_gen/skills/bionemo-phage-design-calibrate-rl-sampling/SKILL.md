...
Set decoder and calibration ceilings from the reward curve, not its full-credit boundary. When learning termination, place every deployed no-EOD cap a small explicit distance beyond the upper zero so capped outputs receive no length credit without wasting a long flat-zero decode tail. If the sampled population lies below the biological band, a wider lower taper may provide monotonic shaping credit to hard-gate failures; document that choice and retain the separate hard acceptance interval.

For an explicitly approved no-EOD reward-gate experiment, a farther cap may instead separate generation-limit exhaustion from a nearby overshoot. State that this does not add a distance gradient where the length reward is already flat zero, keep the ordinary short-cap policy as the default, and compare the experiment from a fresh result root.
