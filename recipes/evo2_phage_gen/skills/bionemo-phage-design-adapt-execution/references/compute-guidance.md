...
Evo2 has two shape-dependent indexing limits: paged-KV append chooses int32/int64 from the reachable strided storage span, while the local GLU output `microbatch × sequence_length × ceil(ffn_width / TP)` must stay below signed INT32_MAX. DP does not reduce that local span. Round generation context to a cache block above longest tokenized prompt plus `max_new_tokens`, rather than allocating an unrelated larger context.

For the current 26.07 PhiX RL profile, keep the configured `policy.megatron_cfg.attention_backend=fused`: cuDNN completed full-model backward where FA4/CuTe stalled on the tested H100 stack. Change the model setting, not just `NVTE_*` environment selectors, which Evo2 can reset. A short-attention component test is not a substitute for full-model updates, validation and checkpoint reload, nor capacity evidence for a longer genome.
