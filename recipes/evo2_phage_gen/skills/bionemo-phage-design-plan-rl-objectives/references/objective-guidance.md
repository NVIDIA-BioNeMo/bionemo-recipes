...
The maintained PhiX profile implements this distinction through shared curated family evidence and a `core_gene_reference_functions` map: supported alternatives can fill an architecture slot without 90% identity to the reference. See the [function-aware synteny API](../../bionemo-phage-design-implement-rl-objectives/references/reward-api.md#function-aware-synteny) for the mapping, order/copy checks, and the separate reference-specific origin and tropism objectives.
