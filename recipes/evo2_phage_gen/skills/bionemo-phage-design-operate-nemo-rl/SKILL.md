...
Match Ray environment concurrency to the framework call shape. When NeMo-RL submits one already-batched environment call per task, keep the actor at `max_concurrency=1`; a thread per possible row adds no scorer parallelism and can create a host-RSS staircase through glibc arenas. Monitor actor RSS and thread count across updates and through the first optimizer checkpoint in addition to GPU memory.
