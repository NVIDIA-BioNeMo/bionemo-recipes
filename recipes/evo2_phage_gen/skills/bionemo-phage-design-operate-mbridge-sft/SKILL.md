...
For the explicitly requested `phix174_8xh100.sh --quick-e2e` pipeline check, use its bounded
SFT budget and smoke-labeled lowest-loss checkpoint, even when that point is the endpoint.
The convergence/extension rules below apply to scientific training, not this execution test.

For indexed data verified to contain no taxonomy annotations, use `--skip-taxonomy-loss-mask`, as the PhiX launcher does. It bypasses the taxonomy heuristic while retaining non-DNA prefix masking, genuine EOD supervision, and padding exclusion. Leave taxonomy masking enabled for annotated corpora. Keep the setting consistent across train and evaluation; changing it starts a new attempt with a different loss definition.
