...
For architecture objectives, use maximum-weight one-to-one matching against a fixed configured slot denominator (mapped functions for the PhiX profile, callable reference loci for a reference-only profile). Score circular order from the same graded edges, and penalize excess homolog mass so deletion or duplication cannot repair the score. Remove pseudocircular annotation artifacts from the denominator rather than making the reference fail its own gate.
