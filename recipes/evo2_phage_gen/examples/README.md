...
| `synteny`                  | [`score_function_matches` / `smooth_protein_match_integrity`](../src/bionemo/evo2_phage_gen/protein_evidence.py), [`score_smooth_reference_architecture`](../src/bionemo/evo2_phage_gen/protein_evidence.py), and [`_add_smooth_reference_rewards`](../src/bionemo/evo2_phage_gen/reward.py) |
...
- **Final per-genome QC** uses exact safety `PASS` plus the Arc target-profile waterfall: A/C/G/T
  ...
  order with no excess qualifying copies. K and A\* are outside this architecture profile. DUST ≤0.9 is an online/checkpoint
