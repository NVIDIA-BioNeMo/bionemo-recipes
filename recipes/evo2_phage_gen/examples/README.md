...
- **Checkpoint selection** uses
  ...
  credit on the binary-core rewards—including the 5,359–5,391-nt length band and full G-protein
  reciprocal coverage—then the external hard-pass flags, and counts one representative per online
  99%-identity cluster. It is intentionally stricter than final rollout QC in some dimensions.
...
| `dustmask_end` (`reward_dustmask_end`)       | No valid masked fraction reaches zero; failed or invalid DUST measurement scores zero.               | Maximum DUST-masked fraction *F* over either terminal 200-nt window ≤ 0.9.                                                                 | For 0.9 < *F* ≤ 1, score `0.9/F` (0.9–1.0). The online nucleotide-pass objective supplies the binary cutoff; the final Arc target profile does not repeat this gate. DUST detects low-complexity sequence using the approach described by [Morgulis et al.](https://doi.org/10.1089/cmb.2006.13.1028).                                  |
...
| `required_genes` (`reward_external_required_genes`)                     | Missing metrics, missing alignment lengths, an empty required-label definition, or zero evidence.                                                       | All 10 ORFipy-callable gene-copy slots have distinct ORF and PHROG-family evidence at full reciprocal coverage.                              | Score `(coverage_sum/total) × min(total/10,1)`. The hard gate is less strict: all 10 slots must have distinct evidence with ≥0.75 query and target coverage. Nine functional labels are used, with two one-to-one slots for head morphogenesis (B/D). One ORF or PHROG target cannot satisfy both copies; A\* remains outside this ORF-caller-based term, while the architecture gate distinguishes callable reference loci and penalizes extra homolog copies. |
...
| `mmseqs_cluster_diversity` (`reward_mmseqs_cluster_diversity`)          | The genome fails basic nucleotide feasibility, is missing from MMseqs output, or the tool fails. No finite cluster size otherwise reaches exactly zero. | A singleton within its prompt group.                                                                                                         | With 99% minimum sequence identity, a member of a cluster of size *N* scores `1/N`. This rewards within-batch diversity but is not a per-genome hard gate; final passers are clustered separately for set-level selection. [MMseqs2](https://doi.org/10.1038/nbt.3988) provides the clustering implementation.                                                                                                                                                  |
