...
```bash
python recipes/evo2_megatron/src/bionemo/evo2/data/preprocess.py -c <CONFIG_PATH>
```
...
To check equivalence of the two preprocessed datasets, we verify that we get the same elements out of our processed dataset as the original, but do not enforce ordering of the data. (`bionemo.common.fasta` does not sequentially read the `.fasta` file.)
